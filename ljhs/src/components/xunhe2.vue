    ...
    <div class="box">
      <div class="sec1">
        ...
        <van-radio-group v-model="radio" @change="sel_he">
          <div class="c_he">
            <van-radio
              :name="index"
              checked-color="#157FCA"
              v-for="(it,index) in riverList"
              :key="index"
            >
              <span class="font2">{{it.name}}</span>
            </van-radio>
      ...
      <div class="sec1">
        ...
        <div style="display:flex;">
          <img
            src="../assets/images/upcamera.jpg"
            class="cameraIcon"
            @click="getImage"
...
export default {
  ...
  created() {
    // 河段
    this.postRequest({ cmd: "reachList", uid: this.uid }).then(res => {
      // console.log(res)
  ...
  methods: {
    questions() {
      this.postRequest({ cmd: "questionList" }).then(res => {
        if (res.data.dataList) {
          // console.log(res)
    ...
    sel_weather(num) {
      if (num == "1") this.weather = "晴";
      else if (num == "2") this.weather = "多云";
      else this.weather = "阴天";
    },
    choose(n) {
            ...
            function(entry) {
              entry.file(function(file) {
                ...
                fileReader.onloadend = function(f) {
                  newfile = self.dataURLtoFile(
                    f.target.result.toString(),
                    "detail.png"
                  ); //图片文件流
                  self.onRead1(newfile, f.target.result.toString(), obj => {
                    ...
                    axios
                      .post(
                        "http://122.114.48.61:8080/garbage/api/uploadFile",
                        formdata
                      )
                      .then(res => {
